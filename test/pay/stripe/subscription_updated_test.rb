...
class Pay::Stripe::SubscriptionRenewingTest < ActiveSupport::TestCase
  ...
  test "nothing happens if a subscription can't be found" do
    ...
    Pay::Stripe::SubscriptionUpdated.new.call(@event)
  end
  ...
  test "subscription is updated with cancel_at_period_end = true and on_trial? = true" do
    ...
    @event.data.object.stubs(:cancel_at_period_end).returns(true)
    Subscription.any_instance.stubs(:on_trial?).returns(true)
    Subscription.any_instance.stubs(:trial_ends_at).returns(3.days.from_now.beginning_of_day)
