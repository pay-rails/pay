...
class Pay::Stripe::SubscriptionDeletedTest < ActiveSupport::TestCase
  setup do
  ...
  test "it sets ends_at on the subscription" do
    ...
    Subscription.any_instance.expects(:update!).with(ends_at: Time.at(@event.data.object.ended_at))
    Pay::Stripe::SubscriptionDeleted.new.call(@event)
  end

  test "it doesn't set ends_at on the subscription if it's already set" do
    @user = User.create!(email: 'gob@bluth.com', processor: :stripe, processor_id: @event.data.object.customer)
    subscription = @user.subscriptions.create!(processor: :stripe, processor_id: @event.data.object.id, name: 'default', processor_plan: 'some-plan', ends_at: Time.now)

    Subscription.any_instance.expects(:update!).with(ends_at: Time.at(@event.data.object.ended_at)).never
    Pay::Stripe::SubscriptionDeleted.new.call(@event)
  end
