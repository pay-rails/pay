...
class Pay::Billable::Test < ActiveSupport::TestCase
  ...
  test "has subscriptions" do
    assert_equal Pay::Subscription.none, users(:none).subscriptions
  end

  test "customer with processor" do
    assert_equal @user.payment_processor, @user.payment_processor.customer
  end
  ...
  test "customer without processor" do
    assert_raises StandardError do
      users(:none).payment_processor.customer
    end
  end
  ...
  test "retrieves a processor subscription" do
    assert_equal pay_subscriptions(:fake), @user.payment_processor.processor_subscription("fake_1")
  end

  test "getting a processor subscription without a processor" do
    assert_raises StandardError do
      users(:none).payment_processor.processor_subscription("123")
    end
  end
