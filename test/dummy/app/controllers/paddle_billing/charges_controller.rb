class PaddleBilling::ChargesController < ApplicationController
  before_action :set_charge, only: [:show, :refund]

  def index
    @charges = Pay::Charge.joins(:customer).where(pay_customers: {processor: :paddle_billing}).order(created_at: :desc)
  end
  ...
  def create
    ...
    redirect_to paddle_billing_charge_path(charge)
  rescue Pay::Error => e
