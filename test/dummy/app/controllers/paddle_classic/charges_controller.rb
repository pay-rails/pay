class PaddleClassic::ChargesController < ApplicationController
  before_action :set_charge, only: [:show, :refund]

  def index
    @charges = Pay::Charge.joins(:customer).where(pay_customers: {processor: :paddle_classic}).order(created_at: :desc)
  end
