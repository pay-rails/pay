    ...
    module CustomerExtension
      ...
      def set_payment_processor(processor_name, allow_fake: false, stripe_account: nil, **attributes)
        ...
        with_lock do
          ...
          pay_customer = pay_customers.active.where(Pay::Customer.inheritance_column => klass.name, processor: processor_name, stripe_account: stripe_account).first_or_initialize
    ...
    module MerchantExtension
      ...
      def set_merchant_processor(processor_name, **attributes)
        with_lock do
          ...
          pay_merchant = pay_merchants.where(Pay::Merchant.inheritance_column => "Pay::#{processor_name.to_s.classify}::Merchant", processor: processor_name).first_or_initialize
