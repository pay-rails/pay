    ...
    module Webhooks
      class SubscriptionTrialWillEnd
        def call(event)
          ...
          if Pay.send_email?(:subscription_trial_will_end, pay_subscription) && pay_subscription.trial_ends_at.after?(Time.current)
            pay_user_mailer.subscription_trial_will_end.deliver_later
          elsif Pay.send_email?(:subscription_trial_ended, pay_subscription) && pay_subscription.trial_ends_at.before?(Time.current)
